...
public class ArgumentParser
{
    public ParsedArguments Parse(string[] args)
    {
        ...
        {
            throw new ArgumentException("Invalid arguments. Usage: <coreAppPath> <jsonConfigString> [--debug] [--ev] or --json <path to .json file>");
        }
        ...
        for (int i = 0; i < args.Length; i++)
        {
            ...
            else if (args[i].Equals("--ev", StringComparison.OrdinalIgnoreCase))
            {
                parsedArgs.UseEnvironmentVariables = true;
            }
